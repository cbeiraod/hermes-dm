...
from pyvisa.constants import VI_WRITE_BUF_DISCARD


def auto_discover_instrument(rm: pyvisa.ResourceManager, identifier: str) -> Tuple[str, str, str]:
    """
    ...
    try:
        ...
        raise ConnectionError(f"Could not open port {identifier}: {e}") from e
        ...
        for w_term in terminators:
            for r_term in terminators:
                try:
                    ...
                    try:
                        instrument.flush(VI_READ_BUF_DISCARD | VI_WRITE_BUF_DISCARD)
                    except pyvisa.VisaIOError:
                        pass
