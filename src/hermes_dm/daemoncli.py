...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Hermes Device Manager: Continuous logging and control daemon."
    )
    ...
    parser.add_argument(
        ...
        type=str, 
        default="./logs", 
        help="Directory to store the SQLite database files. (Default: ./logs)"
    )
    parser.add_argument(
        "--cmd-port", 
        type=int, 
        default=5555, 
        help="ZeroMQ REP port for receiving commands. (Default: 5555)"
    )
    parser.add_argument(
        "--pub-port", 
        type=int, 
        default=5556, 
        help="ZeroMQ PUB port for streaming live data. (Default: 5556)"
    )
